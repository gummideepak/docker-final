...
def test_transaction_upload(client, application):
    ...
    with application.app_context():
        ...
        testFile = os.path.join(root, 'testfiles/transactions.csv')
        ...
        token = str(client.get('/transactions/upload').data)
        token = extract_csrf_token(token)
        ...
        #     'csrf_token': token
        # }
        # response = client.post('/transactions/upload', data=fileData, follow_redirects=True)
        ...
        '/transactions/upload', data={'file':my_files, 'csrf_token':token}, follow_redirects=True,
        content_type='multipart/form-data'
        )
        assert db.session.query(Transactions).count() == 28
